...
「程式碼與權重」頁面將原有資源卡片排成左側單欄，右側顯示目前所選資源的使用說明，進入時預設選取第一項。點擊卡片或使用鍵盤選取，可切換各自的文字與圖片說明；「取得」按鈕仍會開啟原本的檔案另存流程。現階段說明文字與圖片區域均為明確標示的佔位內容，待補入正式操作步驟與權重放置示意圖。「安裝檔」頁面維持原有排列與操作方式。
